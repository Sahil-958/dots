...
//await Utils.execAsync("ags -r dark.value")
//  .then((res) => {
//    dark = res == "true";
//  })
//  .catch(() => {});
...
Utils.execAsync(`cat ${App.configDir}/primary`)
  .then((res) => {
    colors = res.split(",").map((n) => Number(n) / 255);
  })
  .catch((e) => {
    console.log(e);
  });
...
const NierGeom = ({
  ...
  update_css = async (self) => {
    try {
      if (first_update < 4) {
        first_update++;
        return;
      }
      let [tmp_cell_x1, tmp_cell_y1] = pos_mapper(
        anchor_x1.value,
        anchor_y1.value,
        SCREEN_WIDTH,
        SCREEN_HEIGHT,
      ...
      let [tmp_cell_x2, tmp_cell_y2] = pos_mapper(
        anchor_x2.value,
        anchor_y2.value,
        SCREEN_WIDTH,
        SCREEN_HEIGHT,
      ...
      for (let i = 0; i < rows * cols; i++) {
        ...
        let leftside =
          x < cell_x2 &&
          dist_from_center(x, y, real_x1 + y_dist, center_y, cols, rows) <
            y_dist;
        let rightside =
          x > cell_x1 &&
          dist_from_center(x, y, real_x2 - y_dist, center_y, cols, rows) <
            y_dist;
        let topside =
          x > cell_x1 &&
          x < cell_x2 &&
          y < cell_y2 &&
          dist_from_center(x, y, center_x, real_y1 + x_dist - 2, cols, rows) <
            x_dist;
        let bottomside =
          x > cell_x1 &&
          x < cell_x2 &&
          y > cell_y1 &&
          dist_from_center(x, y, center_x, real_y2 - x_dist + 1, cols, rows) <
            x_dist;
      ...
      }px;margin-bottom:${height - max(anchor_y1.value, anchor_y2.value)}px;`;
    } catch (e) {
      print(e);
    }
  },
        ...
        self.connect("key-press-event", async (widget, event) => {
          try {
            if (event.get_keyval()[1] == Gdk.KEY_Escape) {
              ...
              let max_dist =
                (max(center_x, rows - center_x) ** 2 +
                  max(center_y, cols - center_y) ** 2) **
                  0.5 +
                1;
              while (true) {
                ...
                for (let i = 0; i < rows * cols; i++) {
                  ...
                  if (
                    time_ratio > 1
                      ? 1
                      : dist < max_dist * time_ratio * (rand_int(10, 100) / 100)
                  ) {
                    inited = true;
                    t_opacity = 0;
                  }
                ...
                draw_t = Date.now();
                await new Promise((r) =>
                  setTimeout(r, max(0, 1000 / fps - (draw_t - frame_start))),
                );
              }
            }
            // it ctrl
            if (event.get_keyval()[1] == 65507) {
              ...
              Hyprland.sendMessage(
                `dispatch movecursor ${round(tmp1)} ${round(tmp2)}`,
              );
            }
          } catch (e) {
            print(e);
      ...
      child: Overlay({
                  ...
                  for (let i = 0; i < rows * cols; i++) {
                    ...
                    if (
                      time_ratio > 1
                        ? 1
                        : dist <
                          max_dist * time_ratio * (rand_int(50, 100) / 100)
                    ) {
                      if (!inited) {
                        inited = true;
                        c_opacity = 1;
        ...
        overlays: [
          Box({
            ...
            .hook(anchor_x1, update_css)
            .hook(anchor_x2, update_css)
            .hook(anchor_y1, update_css)
            .hook(anchor_y2, update_css),
        ],
      }),
      ...
      setup: (self) =>
        Utils.timeout(1, () => {
          ...
          self.connect("button-release-event", (self, event) => {
            ...
            Utils.timeout(10, () => {
              print(
                `${round(min(anchor_x1.value, anchor_x2.value))},${round(min(anchor_y1.value, anchor_y2.value))} ${abs(round(anchor_x2.value - anchor_x1.value)) + 1}x${abs(round(anchor_y2.value - anchor_y1.value)) + 1}`,
              );
              App.quit();
            });
          });
...
//export default NierGeom;
Utils.exec(`sass ${App.configDir}/slurp.scss ${App.configDir}/slurp.css`);

//App.applyCss(`${App.configDir}/slurp/slurp.css`);

App.config({
  style: `${App.configDir}/slurp.css`,
  windows: [NierGeom({})],
});
