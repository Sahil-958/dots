...
char_entry=$(
    for char in "${chars[@]}";{
        echo -n "${char/,/   }"
    }|rofi -dmenu -config ~/dots/rofi/config-dmenu.rasi -theme-str '*{font: "ComicShannsMono Nerd Font 18";} 
    listview{columns: 3; fixed-columns: false;} window{ width: 70%; height: 40%;}'
)
