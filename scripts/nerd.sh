...
case $1 in
    update)
        ...
        ;;
    *)
        jq -r '. | to_entries[] | select(.key != "METADATA") | "\(.value.char)   \(.key) \(.value.code)"' ~/dots/scripts/glyphnames.json |rofi -p "NerdFont Glyphs" -dmenu -i -matching regex -config ~/dots/rofi/config-dmenu.rasi -theme-str '
